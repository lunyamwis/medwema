...
class Clinic(models.Model):
    ...
    created_by = models.OneToOneField(User, on_delete=models.CASCADE, related_name="clinic")
    staff = models.ManyToManyField(User, related_name="clinics", blank=True)
...
class ClinicBankDetails(models.Model):
    ...
    ACCOUNT_TYPES = [("bank", "Bank"), ("mobile_money", "Mobile Money")]
    BANKS = [
        (191, "Absa Bank Kenya Plc"),
        (208, "Access Bank Kenya"),
        (211, "African Banking Corporation"),
    ...
    clinic = models.ForeignKey(Clinic, on_delete=models.CASCADE, related_name="bank_details")
    ...
    def __str__(self) -> str:
        return f"{self.clinic.name} — {self.get_bank_name_display()}"
